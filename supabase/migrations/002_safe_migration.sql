-- 🗄️ БЕЗОПАСНАЯ МИГРАЦИЯ - с проверками существования
-- Применять в Supabase Dashboard → SQL Editor

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Функция для автоматического обновления updated_at (если не существует)
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_user_profiles_user_id') THEN
    CREATE INDEX idx_user_profiles_user_id ON user_profiles(user_id);
  END IF;

  -- Индексы для resume_analysis
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_resume_analysis_user_id') THEN
    CREATE INDEX idx_resume_analysis_user_id ON resume_analysis(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_resume_analysis_status') THEN
    CREATE INDEX idx_resume_analysis_status ON resume_analysis(processing_status);
  END IF;

  -- Индексы для work_experience
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_work_experience_user_id') THEN
    CREATE INDEX idx_work_experience_user_id ON work_experience(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_work_experience_current') THEN
    CREATE INDEX idx_work_experience_current ON work_experience(user_id, is_current);
  END IF;

  -- Индексы для education_history
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_education_history_user_id') THEN
    CREATE INDEX idx_education_history_user_id ON education_history(user_id);
  END IF;

  -- Индексы для test_results
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_test_results_user_id') THEN
    CREATE INDEX idx_test_results_user_id ON test_results(user_id);
  END IF;
  
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_test_results_user_date') THEN        
    CREATE INDEX idx_test_results_user_date ON test_results(user_id, test_date);                  
  END IF;
END
$$;
...
-- Создаем политики безопасности (или обновляем существующие)
DO $$
BEGIN
  ...
  -- Политики для resume_analysis
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'resume_analysis' AND policyname = 'Users can view own resume analysis') THEN
    CREATE POLICY "Users can view own resume analysis" ON resume_analysis
      FOR ALL USING (auth.uid() = user_id);
  END IF;

  -- Политики для work_experience
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'work_experience' AND policyname = 'Users can view own work experience') THEN
    CREATE POLICY "Users can view own work experience" ON work_experience
      FOR ALL USING (auth.uid() = user_id);
  END IF;

  -- Политики для education_history
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'education_history' AND policyname = 'Users can view own education history') THEN
    CREATE POLICY "Users can view own education history" ON education_history
      FOR ALL USING (auth.uid() = user_id);
  END IF;

  -- Политики для test_results
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'test_results' AND policyname = 'Users can view own test results') THEN
...
-- Создаем триггер для пересчета процента заполненности
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'calculate_completeness_on_update') THEN
...
SELECT 
  'resume_analysis' as table_name,
  COUNT(*) as column_count
FROM information_schema.columns 
WHERE table_name = 'resume_analysis' AND table_schema = 'public'
...
SELECT 
  ...
  COUNT(*) as column_count
FROM information_schema.columns 
WHERE table_name = 'work_experience' AND table_schema = 'public'

UNION ALL

SELECT 
  'education_history' as table_name,
  COUNT(*) as column_count
FROM information_schema.columns 
WHERE table_name = 'education_history' AND table_schema = 'public'
...
-- Показать созданные политики RLS
