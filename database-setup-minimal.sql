-- Минимальный SQL скрипт для создания таблиц платформы тестирования
-- Этот скрипт создает только основные таблицы без сложных политик
...
-- Простые политики RLS для публичного доступа
...
-- Вставка базовых категорий тестов (без ON CONFLICT)
INSERT INTO test_categories (name, name_ru, description, icon, color, order_index) 
SELECT 'programming', 'Программирование', 'Тесты по программированию и разработке', '💻', '#3B82F6', 1
WHERE NOT EXISTS (SELECT 1 FROM test_categories WHERE name = 'programming');

INSERT INTO test_categories (name, name_ru, description, icon, color, order_index) 
SELECT 'design', 'Дизайн', 'Тесты по дизайну и UX/UI', '🎨', '#EF4444', 2
WHERE NOT EXISTS (SELECT 1 FROM test_categories WHERE name = 'design');

INSERT INTO test_categories (name, name_ru, description, icon, color, order_index) 
SELECT 'marketing', 'Маркетинг', 'Тесты по маркетингу и рекламе', '📈', '#10B981', 3
WHERE NOT EXISTS (SELECT 1 FROM test_categories WHERE name = 'marketing');

INSERT INTO test_categories (name, name_ru, description, icon, color, order_index) 
SELECT 'management', 'Менеджмент', 'Тесты по управлению и лидерству', '👥', '#F59E0B', 4
WHERE NOT EXISTS (SELECT 1 FROM test_categories WHERE name = 'management');

INSERT INTO test_categories (name, name_ru, description, icon, color, order_index) 
SELECT 'analytics', 'Аналитика', 'Тесты по анализу данных и бизнес-аналитике', '📊', '#8B5CF6', 5
WHERE NOT EXISTS (SELECT 1 FROM test_categories WHERE name = 'analytics');
