...
) AS new_categories(name, name_ru, description, icon, order_index)
WHERE NOT EXISTS (
    SELECT 1 FROM digital_skill_categories dsc 
    WHERE dsc.name = new_categories.name
);
...
DECLARE
    ...
    cat_comm_id UUID;
    cat_data_id UUID;
    cat_security_id UUID;
    cat_online_id UUID;
BEGIN
    -- Получаем ID категорий
    SELECT id INTO cat_basic_id FROM digital_skill_categories WHERE name = 'basic_digital_literacy';
    SELECT id INTO cat_office_id FROM digital_skill_categories WHERE name = 'office_applications';
    SELECT id INTO cat_comm_id FROM digital_skill_categories WHERE name = 'communication_tools';
    SELECT id INTO cat_data_id FROM digital_skill_categories WHERE name = 'data_analysis';
    SELECT id INTO cat_security_id FROM digital_skill_categories WHERE name = 'digital_security';
    SELECT id INTO cat_online_id FROM digital_skill_categories WHERE name = 'online_presence';
    
    -- Вставляем навыки только если их еще нет
    INSERT INTO digital_skills (category_id, name, name_ru, description, order_index)
    SELECT * FROM (VALUES
        -- Базовая цифровая грамотность
        (cat_basic_id, 'computer_basics', 'Основы работы с ПК', 'Включение/выключение, файловая система, установка программ', 1),
        (cat_basic_id, 'internet_browsing', 'Работа с интернетом', 'Браузеры, поиск информации, скачивание файлов', 2),
        (cat_basic_id, 'email_usage', 'Электронная почта', 'Отправка, получение, вложения, папки', 3),
        (cat_basic_id, 'mobile_devices', 'Мобильные устройства', 'Смартфоны, планшеты, приложения, синхронизация', 4),
        
        -- Офисные приложения
        (cat_office_id, 'word_processing', 'Текстовые редакторы', 'MS Word, Google Docs, форматирование, стили', 1),
        (cat_office_id, 'spreadsheets', 'Электронные таблицы', 'Excel, Google Sheets, формулы, графики', 2),
        (cat_office_id, 'presentations', 'Презентации', 'PowerPoint, Google Slides, дизайн слайдов', 3),
        (cat_office_id, 'pdf_handling', 'Работа с PDF', 'Просмотр, создание, редактирование PDF-файлов', 4),
        
        -- Коммуникационные инструменты
        (cat_comm_id, 'video_conferencing', 'Видеоконференции', 'Zoom, Teams, Meet, организация встреч', 1),
        (cat_comm_id, 'instant_messaging', 'Мессенджеры', 'WhatsApp, Telegram, корпоративные чаты', 2),
        (cat_comm_id, 'collaboration_platforms', 'Платформы сотрудничества', 'Slack, Trello, Notion, совместная работа', 3),
        (cat_comm_id, 'calendar_management', 'Управление календарем', 'Google Calendar, Outlook, планирование встреч', 4),
        
        -- Работа с данными
        (cat_data_id, 'excel_advanced', 'Продвинутый Excel', 'Сводные таблицы, макросы, сложные формулы', 1),
        (cat_data_id, 'data_visualization', 'Визуализация данных', 'Графики, диаграммы, дашборды', 2),
        (cat_data_id, 'basic_statistics', 'Базовая статистика', 'Среднее, медиана, корреляция, тренды', 3),
        (cat_data_id, 'database_basics', 'Основы баз данных', 'SQL запросы, фильтрация, сортировка', 4),
        
        -- Цифровая безопасность
        (cat_security_id, 'password_management', 'Управление паролями', 'Сложные пароли, менеджеры паролей, 2FA', 1),
        (cat_security_id, 'phishing_awareness', 'Распознавание фишинга', 'Подозрительные письма, ссылки, социальная инженерия', 2),
        (cat_security_id, 'data_backup', 'Резервное копирование', 'Облачные хранилища, локальные копии, восстановление', 3),
        (cat_security_id, 'privacy_settings', 'Настройки приватности', 'Социальные сети, браузеры, персональные данные', 4),
        
        -- Онлайн-присутствие
        (cat_online_id, 'social_media', 'Социальные сети', 'Facebook, Instagram, профессиональное использование', 1),
        (cat_online_id, 'linkedin_usage', 'LinkedIn', 'Профиль, нетворкинг, поиск работы', 2),
        (cat_online_id, 'online_reputation', 'Цифровая репутация', 'Управление репутацией, поиск себя в интернете', 3),
        (cat_online_id, 'digital_etiquette', 'Цифровой этикет', 'Культура общения онлайн, профессиональная переписка', 4)
    ) AS new_skills(category_id, name, name_ru, description, order_index)
    WHERE NOT EXISTS (
        SELECT 1 FROM digital_skills ds 
        WHERE ds.category_id = new_skills.category_id 
        AND ds.name = new_skills.name
    );
END $$;
...
) AS new_regions(name, name_ru, federal_district)
WHERE NOT EXISTS (
    SELECT 1 FROM regions r 
    WHERE r.name = new_regions.name
);
...
) AS new_professions(name, name_ru, description, industry, is_ict_related)
WHERE NOT EXISTS (
    SELECT 1 FROM professions p 
    WHERE p.name = new_professions.name
);

-- 5. ПРИМЕРЫ ОБРАЗОВАТЕЛЬНЫХ РЕСУРСОВ
INSERT INTO learning_resources (title, description, provider, url, resource_type, target_skills, duration_hours, price_rub, difficulty_level, rating)
SELECT * FROM (VALUES
...
) AS new_resources(title, description, provider, url, resource_type, target_skills, duration_hours, price_rub, difficulty_level, rating)
WHERE NOT EXISTS (
    SELECT 1 FROM learning_resources lr 
    WHERE lr.title = new_resources.title
);
